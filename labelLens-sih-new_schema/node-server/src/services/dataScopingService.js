 ...
 * - REVIEWER: Queries must append where: { reviewerId: req.user.id }
 * - CONTROLLER: Queries must filter by jurisdiction: where: { inspector: { district: req.user.district } }
    ...
    case ROLES.CONTROLLER:
      if (!user.district || !String(user.district).trim()) {
        throw new SecurityScopingError(
          "CONTROLLER account is missing assigned jurisdiction (district). Access denied to prevent data leakage.",
          { role, userId: user.id }
        );
      }
      return {
...
export function getComplaintScope(user) {
  ...
  switch (role) {
    ...
    case ROLES.CONTROLLER:
      if (!user.district) {
        throw new SecurityScopingError("CONTROLLER missing district for complaint triage");
      }
      return {
        district: {
          equals: String(user.district).trim(),
...
export function getReportScope(user) {
  ...
  switch (role) {
    ...
    case ROLES.CONTROLLER:
      if (!user.district) {
        throw new SecurityScopingError("CONTROLLER missing jurisdiction district for report scoping");
      }
      return {
        inspection: {
          inspector: {
...
export function canAccessInspection(user, inspection) {
  ...
  switch (role) {
    ...
    case ROLES.REVIEWER:
      return inspection.reviewerId === user.id;
